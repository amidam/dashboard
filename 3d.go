...
func bar3DBase(w http.ResponseWriter, _ *http.Request) {
	bar3d := charts.NewBar3D()
	bar3d.SetGlobalOptions(
		charts.WithTitleOpts(opts.Title{Title: "نمودار خطی ۳ بعدی"}),
...
func bar3DAutoRotate(w http.ResponseWriter, _ *http.Request) {
	bar3d := charts.NewBar3D()
	bar3d.SetGlobalOptions(
		charts.WithTitleOpts(opts.Title{Title: "نمودار خطی ۳ بعدی با چرخش خودکار"}),
		charts.WithVisualMapOpts(opts.VisualMap{
			Calculable: true,
			Max:        30,
			Range:      []float32{0, 30},
